...
static struct _JVM_Handle_stack *JVM_Handle_stack = NULL;




static JNI_Class_Stack *jni_methods_stack = NULL;

JVM_Handle *Get_JVM_Handle(JavaVM *vm){
	if (JVM_Handle_stack == NULL)return NULL;
	else {
		struct _JVM_Handle_stack *jvm_handle_stack_head = JVM_Handle_stack, *jvm_handle_stack = jvm_handle_stack_head;
...
JVM_Handle *Create_JVM_Handle(JavaVM *vm, char * Main_ClassName){
	...
	if (JVM_Handle_stack == NULL){
		jvm_handle_stack->next = jvm_handle_stack;
		JVM_Handle_stack = jvm_handle_stack;
	}
	else{
		jvm_handle_stack->next = JVM_Handle_stack->next;
		JVM_Handle_stack->next = jvm_handle_stack;
	}
	return jvm_handle;
}
...
VM_func_stack * Create_VM(JavaVMOption options[], char * Main_ClassName){
	...
	if (status != JNI_ERR)return alloc_VM_func_stack(jvm, Main_ClassName);
	else{
