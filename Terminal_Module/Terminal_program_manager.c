...
Runing_program* Mark_program(Terminal_data * const terminal, Program func){
	...
	if (Program_list == NULL){
		Runing_program program_1 = { Create_Receipt(func, SUCCESS, Terminal_module_owner, "run succes"), terminal, 0, rand() };
		memmove(program, &program_1, sizeof(Runing_program));
	}
	else{
		Program_stack *i = Program_list;
		while (i->next != NULL)i = i->next;
		Runing_program program_1 = { Create_Receipt(func, SUCCESS, Terminal_module_owner, "run succes"), terminal, i->Program->PID + 1, 1 };
		memmove(program, &program_1, sizeof(Runing_program));
	}
	return program;
}
	...
	if (Program_list != NULL){
		...
		while (i->next != NULL){
			if (i->Program->PID == pid){
				ii->next = i->next;
				free(i);
				receipt = Create_Receipt(Kill_program, ERROR, Terminal_module_owner, "kill program succes.");
				break;
			}
			else{
				ii = i;
		...
		if (receipt == NULL) return Create_Receipt(Kill_program, ERROR, Terminal_module_owner, "kill program failed.");
		else return receipt;
	}
	else return Create_Receipt(Kill_program, WARRING, Terminal_module_owner, "Program list is empty");
}
